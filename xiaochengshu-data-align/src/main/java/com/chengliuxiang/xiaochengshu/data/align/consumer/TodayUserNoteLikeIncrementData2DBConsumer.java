...
import com.chengliuxiang.xiaochengshu.data.align.model.dto.LikeUnlikeNoteMqDTO;
...
@Component
@RocketMQMessageListener(consumerGroup = "xiaochengshu_group_data_align_" + MQConstants.TOPIC_COUNT_NOTE_LIKE,
        topic = MQConstants.TOPIC_COUNT_NOTE_LIKE)
@Slf4j
public class TodayUserNoteLikeIncrementData2DBConsumer implements RocketMQListener<String> {
    ...
    public void onMessage(String body) {
        ...
        script.setScriptSource(new ResourceScriptSource(new ClassPathResource("/lua/bloom_today_user_note_like_check.lua")));
        script.setResultType(Long.class);
        Long result = null;

        // ------------------------- 笔记的点赞数变更记录 ---------------------------
        String noteBloomKey = RedisKeyConstants.buildBloomNoteLikeListKey(date);
        ...
        // ------------------------- 笔记发布者获得的点赞数变更记录 ---------------------------
        String userBloomKey = RedisKeyConstants.buildBloomUserLikeListKey(date);
