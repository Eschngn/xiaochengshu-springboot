...
public class FollowingCountShardingXxlJob {
    ...
    public void followingCountShardingJobHandler() throws Exception {
        ...
        // 死循环
        for (; ; ) {
            List<Long> userIds = selectRecordMapper.selectBatchFromDataAlignFollowingCountTampTable(tableNameSuffix, batchSize);
            if (CollUtil.isEmpty(userIds)) break;
            userIds.forEach(userId -> {
                int followingCount = selectRecordMapper.selectCountFromFollowingTableByUserId(userId);
                int count = updateRecordMapper.updateUserFollowingTotalByUserId(userId, followingCount);
                if (count > 0) {
                    String countUserKey = RedisKeyConstants.buildCountUserKey(userId);
                    boolean isExisted = redisTemplate.hasKey(countUserKey);
                    if (isExisted) {
                        redisTemplate.opsForHash().put(countUserKey, RedisKeyConstants.FIELD_FOLLOWING_TOTAL, followingCount);
                    }
                }
            });
            // 在日增量表中物理删除这一批次记录
